...
//Environment charateristic only 2 byte long (inverter MSB with LSB)
uint8_t HW_Service_Sens_UUID[2] ={0x1A,0x18};
uint8_t TemperatureChar_UUID[2] = {0x6E,0x2A};//UUID short for Temperature Charateristic
uint8_t PressureChar_UUID[2] = {0x6D,0x2A};//UUID short for Pressure Charatersitic
...
//								   sizeof(local_name), local_name, 0, NULL, 0, 0);
 ...
 //pressure charateristic
  ret =  aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_16,PressureChar_UUID ,
                               4,
                               CHAR_PROP_NOTIFY|CHAR_PROP_READ,
                               ATTR_PERMISSION_NONE,
                               GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
  ...
  //Humidity charateristic
  ret =  aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_16,HumidityChar_UUID ,
                                2,
                                CHAR_PROP_NOTIFY|CHAR_PROP_READ,
                                ATTR_PERMISSION_NONE,
                                GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
...
tBleStatus Update_Sensors_Data(void)
{
	...
	Pc = pt.Press *10;
	Press[0]= (uint8_t) Pc;
	Press[1]= (uint8_t) (Pc >>8);
	Press[2] = (uint8_t) (Pc >> 16);
	Press[3] = (uint8_t)(Pc >> 24); //4 byte of pressure
	sprintf(str,"T=%d,H=%d,P=%ld\n\r",ttemp,thum,pt.Press);
	HAL_UART_Transmit(&huart1,(uint8_t *)str,strlen(str),20);
	//Update for temperature
	stat = aci_gatt_update_char_value(HWServW2STHandle, TempCharHandle,0, 2, Temp);
	//Update for Pression
	stat = aci_gatt_update_char_value(HWServW2STHandle, PressCharHandle,0, 4, Press);
	//Update for Humidity
	stat = aci_gatt_update_char_value(HWServW2STHandle, HumCharHandle,0, 2, Humid);
