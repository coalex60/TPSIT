...
uint16_t rx_handle[3]; //array of three handle of characteristics
...
void user_notify(void *pData)
{
	char msg[50]= "EVT_BLUE_GATT_DISC_READ_CHAR_BY_UUID_RESP\n";
	char msg1[50];
...
char output[8];
if (attr_handle == rx_handle[0]+1) suffix = " °C";
 	else if (attr_handle == rx_handle[1]+1) suffix =" mB";
 	else if (attr_handle== rx_handle[2]+1) suffix = " %";
if (Nb_bytes == 2)
	{
